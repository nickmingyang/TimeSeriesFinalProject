...
best_p_wide_ricco = 0
best_q_wide_ricco = 0
max_p_wide_ricco = 100
max_q_wide_ricco = 10
best_cost_wide_ricco = 10000
best_rsme_s0_d1_p_search_wide = 10000.0
best_last36_ase_wide = 10000.0
# to reduce run-time complexity, do a wide search first
for(q_try in seq(0, max_q_wide_ricco, 2)) {
  for (p_try in seq(1, max_p_wide_ricco, 5)) {
    was_error = FALSE
    tryCatch(expr = {
...
max_p_ricco = 100
max_q_ricco = 10
best_cost_ricco = 10000
best_rsme_s0_d1_p_search = 10000.0
best_last36_ase = 10000.0
best_phi_estimates = NULL
best_theta_estimates = NULL
# to reduce run-time complexity, do a wide search first
for(q_try in seq(0, max_q_ricco, 2)) {
  for (p_try in seq(1, max_p_ricco, 5)) {
    was_error = FALSE
    tryCatch(expr = {
      capture.output(est_p_q_search_ricco <- est.arma.wge(AVG_E_i1_ricco, p=p_try, q=q_try), file='NUL')    
    },
    ...
    # weight last 36 higher because they are the forecasts we should care about most
    cost_ricco = (rolling.res.long_ricco$rwRMSE + prediction_ase_ricco)/2
    if(cost_ricco < best_cost_ricco) {
      best_cost_ricco = cost_ricco
      best_rsme_s0_d1_p_search = rolling.res.long_ricco$rwRMSE
      best_last36_ase = prediction_ase_ricco
      best_p_ricco = p_try
      best_phi_estimates = est_p_q_search_ricco$phi
      best_theta_estimates = est_p_q_search_ricco$theta
    }
  }
}
print(paste("best long + immediate rolling window p with wide search was:", best_p_ricco))
print(paste("best long + immediate rolling window q with wide search was:", best_q_ricco))


# best P was p=72 WOW! A huge and complex model BUT it was rolling window RSME verified!
#  I wonder how the ARIMA(72,1,0) would do on future, unseen data. it’s probably just REALLY good at fitting the signal we have, even if we rolling window verify. true crossvalidation would be better, if we had the data.
print(paste("best long rolling (36) ase was:", best_rsme_s0_d1_p_search^2, "for p:", best_p_ricco, "for q:", best_q_ricco, "and d = 1"))
f_ricco = fore.aruma.wge(df$AVG_EP, phi=best_phi_estimates, theta=best_theta_estimates, d = 1, s=0, n.ahead=36, lastn=TRUE)
prediction_ase_ricco = mean((df$AVG_EP[(266-35):266]-f$f)^2) 
